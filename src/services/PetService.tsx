...
/**
 * CRUD actions that update the global atom.
 * Use inside components: const { refresh, create, update, remove } = usePetsActions();
 */
/*
export function usePetsActions() {
    const [pets, setPets] = useAtom(allPetsAtom);
...
/!**
 * Helper to fetch a single pet by id (doesn't touch the list atom).
 * Use on the detail page when not found in cache.
 *!/
export async function getPetById(id: string): Promise<PetDto> {
    const res = await MyApi.getPetById.petGetPetById({ id });
    return res.data;
